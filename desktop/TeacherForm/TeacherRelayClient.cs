...
internal sealed class TeacherRelayClient : ITeacherSessionTransport
{
    ...
    {
        return SendCommandAsync(connectionId, MessageType.RemoteControlStart, new() { ["targetConnectionId"] = connectionId });
    }

    public Task SendRemoteControlStopAsync(string? connectionId)
    {
        return SendCommandAsync(connectionId, MessageType.RemoteControlStop, new() { ["targetConnectionId"] = connectionId ?? "" });
    }
    ...
    {
        return SendCommandAsync(null, MessageType.TeacherBroadcastStop, new() { ["mode"] = "broadcast" });
    }

    public Task SelectWebcamAsync(string connectionId, string cameraId)
    {
        return SendCommandAsync(connectionId, MessageType.WebcamSelect, new() { ["cameraId"] = cameraId });
    }

    public Task RequestWebcamDevicesAsync(string connectionId)
    {
        return SendCommandAsync(connectionId, MessageType.WebcamDevices, new() { ["request"] = "rescan" });
    }
    ...
    private async Task ReceiveLoopAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                if (_clients.TryGetValue(ResolveConnectionId(frame.Envelope), out RelayStudentContext? client))
                {
    ...
    private async Task AcceptStudentHelloAsync(ReceivedFrame hello)
    {
        ...
        {
            await SendToStudentAsync(connectionId, MessageType.Error, new()
            {
        ...
        if (_activeStudentCodes.TryGetValue(normalizedCode, out string? activeConnectionId) &&
            _clients.TryGetValue(activeConnectionId, out RelayStudentContext? activeClient) &&
            activeClient.State.IsOnline)
        {
            ...
            _log($"Từ chối kết nối relay trùng mã sinh viên {studentCode}.");
            return;
        }
        ...
        PolicySnapshot policy = _policyFactory();
        await SendToStudentAsync(state.TransportId, MessageType.HelloAck, policy.ToMetadata());
        await SendToStudentAsync(state.TransportId, MessageType.PolicyUpdate, policy.ToMetadata());
    }
    ...
    private void MarkDisconnected(RelayStudentContext client, string reason)
    {
        ...
        _submissionReceiver.AbortStudent(_sessionId, client.State.StudentCodeOrId);
        if (_clients.TryGetValue(client.State.TransportId, out RelayStudentContext? active) &&
            ReferenceEquals(active, client))
        {
            _clients.Remove(client.State.TransportId);
            _activeStudentCodes.Remove(NormalizeStudentCode(client.State.StudentCodeOrId));
        }
    }
    ...
    private Task SendToStudentAsync(string studentId, string messageType, Dictionary<string, string> metadata, byte[]? payload = null)
    {
        return SendEnvelopeAsync(
            FramedSocketProtocol.CreateEnvelope(messageType, _sessionId, studentId, Environment.MachineName, metadata, studentId),
            payload,
